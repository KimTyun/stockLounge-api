...
router.post('/write', upload.single('file'), async (req, res, next) => {
  ...
  try {
    ...
    const ban = await Ban.findAll({ transaction })
...
router.post('/:id/like', isLoggedIn, async (req, res, next) => {
  ...
  try {
    ...
    if (existLike) {
      // 좋아요 취소
      await existLike.destroy({ transaction })
      const likeCount = await BoardLike.count({
        where: { board_id: boardId },
        transaction,
      })
      await board.update({ like_count: likeCount }, { transaction })
      ...
      // 좋아요 기능
      await BoardLike.create({ board_id: boardId, user_id }, { transaction })
      const likeCount = await BoardLike.count({
        where: { board_id: boardId },
        transaction,
      })
...
router.post('/comment/:commentId/like', isLoggedIn, async (req, res, next) => {
  ...
  try {
    ...
    if (existLike) {
      ...
      await comment.update({ like_count: likeCount }, { transaction })
